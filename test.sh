...
start_post() { [ "\${USE_EXTERNAL_ARGO:-false}" = "true" ] && [ -n "\${ARGO_TOKEN:-}" ] && { pkill -9 cloudflared >/dev/null 2>&1 || true; sleep 1; ${argo_exec}; } || return 0; }
EOF
...
ExecStartPost=/usr/bin/bash -c 'if [ "\${USE_EXTERNAL_ARGO:-false}" = "true" ] && [ -n "\${ARGO_TOKEN:-}" ]; then pkill -9 cloudflared >/dev/null 2>&1 || true; sleep 1; ${argo_exec}; fi'
${systemd_nice_line}
${io_config}
LimitNOFILE=1000000
